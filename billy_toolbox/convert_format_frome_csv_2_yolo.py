...
# 讀取CSV文件
csv_file = '../FacialLandmarkHeatmap/IR_val_20240319_heatmap.csv'
data = pd.read_csv(csv_file)


# 創建輸出目錄
home_path = os.path.expanduser("~")
output_image_dir = f'{home_path}/dataset/FacialLandmark_for_yolov8-pose/images'
output_label_dir = f'{home_path}/dataset/FacialLandmark_for_yolov8-pose/labels'
...
for index, row in tqdm(data.iterrows(), total=data.shape[0], desc="Processing"):
    ...
    for i in range(0, len(keypoints), 2):
        ...
        adjusted_keypoints.extend([adjusted_x, adjusted_y])

    # 構建txt內容
    txt_content = f"0 {new_cx:.6f} {new_cy:.6f} {new_L:.6f} {new_L:.6f} "
    for i in range(0, len(adjusted_keypoints), 2):
        x, y = adjusted_keypoints[i]/target_size, adjusted_keypoints[i+1]/target_size
        txt_content += f"{x:.6f} {y:.6f} 2.0 "
