...
from ultralytics.utils.metrics import OKS_SIGMA, Altek_LandmarkMetrics, box_iou, kpt_iou, NormalizedMeanError
from ultralytics.utils.plotting import output_to_target, plot_images


class Altek_LandmarkValidator(DetectionValidator):
    """
    A class extending the DetectionValidator class for validation based on a altek_landmark model.

    Example:
        ```python
        from ultralytics.models.yolo.altek_landmark import Altek_LandmarkValidator

        args = dict(model='yolov8n-altek_landmark.pt', data='coco8-altek_landmark.yaml')
    ...
    def __init__(self, dataloader=None, save_dir=None, pbar=None, args=None, _callbacks=None):
        ...
        self.metrics = Altek_LandmarkMetrics(save_dir=self.save_dir, on_plot=self.on_plot)
    ...
    def init_metrics(self, model):
        """Initiate altek_landmark estimation metrics for YOLO model."""
        super().init_metrics(model)
        self.kpt_shape = self.data["kpt_shape"]
        is_altek_landmark = self.kpt_shape == [17, 3]
    ...
    def _prepare_pred(self, pred, pbatch):
        """Prepares and scales keypoints in a batch for altek_landmark processing."""
    ...
    def update_metrics(self, preds, batch):
        ...
        for si, pred in enumerate(preds):
            ...
            if nl:
                ...
                # NME
                self.NME.update(pbatch["kpts"][..., :2], pred_kpts[..., :2])
    ...
    def get_stats(self):
        ...
        results_dict["NMS"] = self.NME.compute()#.item()
        return results_dict
    
    def print_results(self):
        """Prints training/validation set metrics per class."""
        pf = "%22s" + "%11i" * 2 + "%11.3g" * len(self.metrics.keys) + "%11.3g"  # print format
        LOGGER.info(pf % ("all", self.seen, self.nt_per_class.sum(), *self.metrics.mean_results(), self.NME.compute()))
