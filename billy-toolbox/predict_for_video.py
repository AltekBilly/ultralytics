...
root = '/home/ubuntu/dataset/WYMY_High_Value_Room_Video_20240816-obb/video'
task = 'obb'
name = 'wiwynn-High_Value_Room-obb-20241202-5'
video_name = '20240816_162752_CBE3_B8A44F494ED7_M1171184-001$A-RDimm.mkv'
video_path = os.path.join(root, video_name)

def main():
    # Load a model
    model = YOLO(f'/home/ubuntu/repo/ultralytics/runs/{task}/{name}/weights/best.pt') # load a custom model
    model.conf = 0.5  # 設定信心閾值 (0.0 - 1.0)
    model.iou = 0.6   # 設定 IoU 閾值 (0.0 - 1.0)

    cap = cv2.VideoCapture(video_path)

    # 設定影片輸出 (可選)
    output_path = os.path.join(root, f"{Path(video_path).stem}_predict-{task}.mp4")
    ...
    while cap.isOpened():
        ...
        for result in results:
            ...
            objs = result.boxes if task == 'detect' else result.obb
            for obj in objs:
